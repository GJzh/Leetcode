...
class ExamRoom(object):
    ...
    def __init__(self, N):
        ...
        self.students = {}
        self.intervals = []
        heapq.heappush(self.intervals, (-(N/2), 0, -1, N))
        self.students[-1] = [-2,N]
    ...
    def seat(self):
        ...
        while len(self.intervals):
            negDist, pos, start, end = self.intervals[0]
        ...
        if start == -1:
            ...
            heapq.heappush(self.intervals, (-dist,  (end+pos) / 2, pos, end))
        elif end == self.N:
            pos = self.N-1
            heapq.heappush(self.intervals, (-((pos-start)/2), (pos+start) / 2, start, pos))
        else:
            pos = (end + start) / 2
            heapq.heappush(self.intervals, (-((pos-start)/2), (pos+start) / 2, start, pos))
            heapq.heappush(self.intervals, (-((end-pos)/2), (end+pos) / 2, pos, end))
    ...
    def leave(self, p):
        ...
        heapq.heappush(self.intervals, (-dist, (a+b)/2, a, b))
